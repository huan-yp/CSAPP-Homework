...
const int BUILTIN_QUIT = 1;
const int BUILTIN_JOBS = 2;
const int BUILTIN_FG = 3;
const int BUILTIN_BG = 4;
...
volatile int foreGroundRunning=0;
...
void eval(char *cmdline)
{
	...
	if (!builtin_cmd(argv)) {
		...
		if(man_id){
			// main
			// if(bg)
			addjob(jobs, man_id, bg?BG:FG, cmdline);
			Sigprocmask(SIG_UNBLOCK, &mask_all, NULL);
			// pid_t pgid = getpid();
			// setgpid(man_id, pgid);
			if(bg == 0){
				// nanosleep(100000, NULL); // sleep for graceful output
				// printf("[%d] (%d) %s", pid2jid(man_id), man_id, cmdline);
				// fflush(stdout);
				waitfg(man_id);
			}
			else{
		...
		// is builtin, no background mode
		int cmd_id = builtin_cmd(argv);
		if(cmd_id == BUILTIN_JOBS){
			listjobs(jobs);
		}
		if(cmd_id == BUILTIN_QUIT){
			quit();
		}
		if(cmd_id == BUILTIN_BG || cmd_id == BUILTIN_FG){
			do_bgfg(argv);
		}
	}
 ...
 *    it immediately.
 * done only return value not run it
 */
int builtin_cmd(char **argv)
{
	/* your code here */
	...
	if (!strcmp(argv[0], "quit"))return BUILTIN_QUIT;
...
void waitfg(pid_t pid)
{
	...
	/* SIGCHILD COULD NOT check if it's the fg job */

	/* your code here */	
	// busy loop
	while(1){
		// pause();
		if(foreGroundRunning == 0){
			// printf("running");
			// fflush(stdout);
			break;
		}
...
void sigint_handler(int sig)
{
	...
	for(int i = 0; i < MAXJOBS; i++) {
		if (jobs[i].state == FG) {
			...
			// child job will be processed by sigchld_handler
		}
	}
	return ;
}
...
/* listjobs - Print the job list, system implementation */
void listjobs(struct job_t *jobs)
{
	int i;
...
/*
 * usage - print a help message, system implementation
 */
void usage(void)
{
