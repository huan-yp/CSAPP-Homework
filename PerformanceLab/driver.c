 ...
 */
static int check_pinwheel(int dim)
{
    int err = 0;
    int i, j;
    int badi = 0;
 ...
 */
static int check_motion(int dim) {
	int err = 0;
    int i, j;
    int badi = 0;
    int badj = 0;
    ...
    for (i = 0; i < dim; i++) {
	for (j = 0; j < dim; j++) {
	    pixel motioned = check_weighted_sum(dim, i, j, orig);
	    if (compare_pixels(result[RIDX(i,j,dim)], motioned)) {
		err++;
		badi = i;
		badj = j;
